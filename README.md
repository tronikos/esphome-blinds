...
All settings are `substitutions`, so you override them in your own config
without touching the package:

| Substitution               | Default | Description                                                                   |
|----------------------------|---------|-------------------------------------------------------------------------------|
| `stepper_dir_pin`          | `D6`    | A4988 DIR pin.                                                                |
| `stepper_step_pin`         | `D7`    | A4988 STEP pin.                                                               |
| `stepper_enable_pin`       | `D5`    | A4988 ENABLE pin (driven as an inverted sleep pin).                           |
| `stepper_dir_pin_inverted` | `false` | Flips the direction, i.e. whether closed is `/` or `\`.                       |
| `stepper_max_speed`        | `250`   | Steps per second.                                                             |
| `stepper_endstop`          | `650`   | Steps between fully closed and fully open. Calibrate with **Reset position**. |
| `stepper_acceleration`     | `2000`  | Steps/s². Use `inf` for an instant start.                                     |
| `stepper_deceleration`     | `2000`  | Steps/s². Use `inf` for an instant stop.                                      |

The package restores the blind position across reboots via the template cover's
`restore_mode: RESTORE`. On an `esp8266` that only survives a power cycle if you
set `restore_from_flash: true`, which is why the instructions below change the
`esp8266` block.

The **Reset position** button deliberately drives the stepper past the endstop
so it re-homes against the physical stop, then returns to fully closed. Use it
after a power loss or if the blinds have drifted.
...
8. Your configuration should now look something like the following. Keep the
   `api`, `ota` and `ap` values the ESPHome dashboard generated for you; they
   are per-device secrets and should not be copied between devices or shared.
   ...
   api:
     encryption:
       key: "<the key the ESPHome dashboard generated for you>"

   ota:
     - platform: esphome
       password: "<the password the ESPHome dashboard generated for you>"
